...
plot_data <- lang_data$data %>% 
  ...
  group_by(Year) %>% 
  summarise(av_eng_pre = mean(percent_english))
...
discharge <- readNWISuv(siteNumber = c("11118500","11113500"), 
                    parameterCd_vt, 
                    startDate_vt, 
                    endDate_vt) %>% renameNWISColumns(discharge) #takes the names
...
```
2. The best way to query multiple data sets is to concatinate them, put them into a verctor and upload them together
